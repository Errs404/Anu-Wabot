...
resolve(result)
     ...
     async sendButton(jid, message, type, button = [], opt = {}) {
        message = (
            await this.prepareMessage(`0@s.whatsapp.net`, message, type, { ...opt }).catch(
                async () => {
                    return { message: await this.prepareMessageContent(message, type, { ...opt }) };
                },
            )
        ).message;
	...
	return await this.sendMessage(id, buffers.data, msgtype, {quoted: qu, mimetype: mimtype, filename:filename,caption:caption, contextInfo:{"mentionedJid": this.getMentionedJidList(caption)}, ...opt})
     ...
     async sendMessage(id, message, type, options = {}) {
        if (type == 'imageMessage') options = {thumbnail:Buffer.alloc(0),...options}
        let waMessage = await this.prepareMessage(id, message, type, options);
